...
func resourceWinDNSRecordCreate(d *schema.ResourceData, m interface{}) error {
	...
	switch record_type {
		case "A":
			...
			psCommand = "Add-DNSServerResourceRecord -ZoneName " + zone_name + " -" + record_type + " -Name " + record_name + " -IPv4Address " + ipv4address + " -ComputerName " + client.domain_controller
		case "CNAME":
			if hostnamealias == "" {
				return errors.New("Must provide hostnamealias if record_type is 'CNAME'")
			}
			psCommand = "Add-DNSServerResourceRecord -ZoneName " + zone_name + " -" + record_type + " -Name " + record_name + " -HostNameAlias " + hostnamealias + " -ComputerName " + client.domain_controller
		default:
...
func resourceWinDNSRecordRead(d *schema.ResourceData, m interface{}) error {
	...
	var psCommand string = "try { $record = Get-DnsServerResourceRecord -ZoneName " + zone_name + " -RRType " + record_type + " -Name " + record_name + " -ComputerName " + client.domain_controller + " -ErrorAction Stop } catch { $record = '''' }; if ($record) { write-host 'RECORD_FOUND' }"
	_, err := goPSRemoting.RunPowershellCommand(client.username, client.password, client.server, psCommand, client.usessl, client.usessh)
	if err != nil {
...
func resourceWinDNSRecordDelete(d *schema.ResourceData, m interface{}) error {
	...
	var psCommand string = "Remove-DNSServerResourceRecord -ZoneName " + zone_name + " -RRType " + record_type + " -Name " + record_name + " -ComputerName " + client.domain_controller + " -Confirm:$false -Force"
